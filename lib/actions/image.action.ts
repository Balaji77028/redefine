...
const populateUser = (query: any) =>
  query.populate({
    path: "author",
    model: User,
    select: "_id fristName lastName clerkId",
  });
...
export async function updateImage({ image, userId, path }: UpdateImageParams) {
  try {
    ...
    const upadtedImage = await Image.findByIdAndUpdate(
      imageToUpdate._id,
      image,
      { new: true }
    );
    ...
    return JSON.parse(JSON.stringify(upadtedImage));
  } catch (error) {
    handleError(error);
  }
}
...
export async function getAllImages({
  ...
  try {
    ...
    let expression = "folder=redefine";
